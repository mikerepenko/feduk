...
public class AnimHappy : MonoBehaviour {
	...
	void Update()
	{
			...
			if (timer < 5)
			{
					if (gameObject.transform.position.y != 4f)
							transform.position = Vector3.MoveTowards(transform.position, new Vector3(transform.position.x, 4f, transform.position.z), speed * 0.02f);
			}
			else
			{
